...
class Classifier(nn.Module):
	...
	def forward(self, batch_embs):
		...
		return self.fc_out(batch_embs) # [batch_size, num_classes]



def build_dataset(datadir, class_text_fn, class_label_fn, sep, embdir, time_list, config):
	...
	for t in time_list:
		...
		# load data
		data_fn = os.path.join(datadir, class_text_fn, sep, str(t)+'.txt')
		with open(data_fn, 'r') as fin:
			for line in fin:
				line = line.strip().split(' ')[:config.len_seq]
				data = np.zeros(config.emb_size)
				cnt = 0
				for word in line:
					if word in wv:
						data += wv[word]
		...
		# load label
		label_fn = os.path.join(datadir, class_label_fn, sep, str(t)+'.txt')
...
def build_dataset_t(datadir, class_text_fn, class_label_fn, sep, embdir, time_list, t, config):
...
def train(datadir, class_text_fn, class_label_fn, embdir, dev_dataset, test_dataset, time_list, config):
	...
	for t in time_list:
		...
		dataset_t = build_dataset_t(datadir, class_text_fn, class_label_fn, 'train', embdir, time_list, t, config)
...
if __name__=='__main__':
	...
	parser.add_argument("--text", default="model")
	...
	class_text_fn = args.text
	class_label_fn = args.label
	...
	if is_dev:
		dev_dataset = build_dataset(datadir, class_text_fn, class_label_fn, 'dev', embdir, time_list, config)
	else:
		dev_dataset = None

	if is_test:
		test_dataset = build_dataset(datadir, class_text_fn, class_label_fn, 'test', embdir, time_list, config)
	else:
		test_dataset = None
	
	print('built dataset! [dev={}] [test={}]'.format(is_dev, is_test))

	dev_results, test_results = train(datadir, class_text_fn, class_label_fn, embdir, dev_dataset, test_dataset, time_list, config)
